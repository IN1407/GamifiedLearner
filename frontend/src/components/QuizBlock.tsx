...
export default function QuizBlock({
  ...
  return (
    <section aria-label={quiz.title ?? 'Quiz'} className="my-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {question.kind === 'mcq' ? (
            ...
            <div className="mt-2">
              ...
              <input
                ...
                className="w-full max-w-md rounded-xl border border-slate-300 px-3 py-2 text-sm focus:border-indigo-400 disabled:bg-slate-50"
              />
              {state.resolved && (
                <p className="mt-1 text-sm text-slate-500">
                  Accepted answer: <code className="rounded bg-slate-100 px-1">{answerText}</code>
                </p>
              )}
            </div>
          )}
