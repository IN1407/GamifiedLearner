...
export default function GradientDescentViz() {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-[#fcfcfb] p-4">
      <p className="text-sm font-semibold text-[#0b0b0b]">Gradient descent playground</p>
      <p className="mb-2 text-xs text-[#52514e]">
        x ← x − η·f′(x) on a non-convex loss. Watch the learning rate change everything.
      </p>

      <div className="flex flex-wrap items-center gap-2 text-xs text-[#52514e]" aria-hidden>
      ...
      <div className="overflow-x-auto">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-[560px]" role="img" aria-label="Loss curve with gradient descent trajectory">
          {/* recessive grid */}
          {[0, 2, 4, 6, 8].map((y) => (
            <line key={y} x1={PAD.l} x2={W - PAD.r} y1={sy(y)} y2={sy(y)} stroke="#e1e0d9" strokeWidth="1" />
          ))}
          <line x1={PAD.l} x2={W - PAD.r} y1={sy(yMin)} y2={sy(yMin)} stroke="#c3c2b7" strokeWidth="1" />
          {[-4, -2, 0, 2, 4].map((x) => (
            <text key={x} x={sx(x)} y={H - 8} textAnchor="middle" fontSize="10" fill="#898781">
              {x}
            </text>
          ))}
          {[0, 2, 4, 6, 8].map((y) => (
            <text key={y} x={PAD.l - 6} y={sy(y) + 3} textAnchor="end" fontSize="10" fill="#898781">
              {y}
            </text>
          ))}
      ...
      <div className="mt-2 grid gap-3 sm:grid-cols-2">
        <label className="text-xs text-[#52514e]">
          Learning rate η = <span className="font-mono font-semibold text-[#0b0b0b]">{lr.toFixed(2)}</span>
          <input
            type="range"
            min="0.01"
            max="1.2"
        ...
        <label className="text-xs text-[#52514e]">
          Start x₀ = <span className="font-mono font-semibold text-[#0b0b0b]">{start.toFixed(1)}</span>
          <input
            type="range"
            min={X_MIN + 0.4}
            max={X_MAX - 0.4}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        ...
        <button onClick={reset} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-50">
          Reset
        </button>
        <span className="text-xs text-[#52514e]" aria-live="polite">
          step {steps} · x = {current.toFixed(3)} · f(x) = {f(current).toFixed(3)}
          {diverging && <strong className="ml-1 text-[#d03b3b]">diverged — η too high!</strong>}
        </span>
      </div>
