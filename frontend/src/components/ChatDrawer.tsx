    ...
    <>
      ...
      {open && (
        <div
          ...
          className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-slate-800 bg-slate-900 shadow-2xl"
        >
          <header className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
            <div>
              <h2 className="font-bold text-slate-100">Course assistant</h2>
              <p className="text-xs text-slate-400">Scoped to: {lessonTitle}</p>
            </div>
            <button
          ...
          <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto p-4">
            ...
            {turns.map((t, i) =>
              t.role === 'user' ? (
                ...
                <div key={i} className="mr-4 rounded-2xl rounded-bl-sm border border-slate-800 bg-slate-800/60 p-3">
                  <Markdown md={t.content} />
                </div>
              ),
            )}
            {busy && (
              <div className="mr-4 flex items-center gap-2 rounded-2xl border border-slate-800 bg-slate-800/60 p-3 text-sm text-slate-400">
                <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-slate-700 border-t-indigo-600" />
                Thinking…
              </div>
            )}
          ...
          <footer className="border-t border-slate-800 p-3">
            <div className="flex gap-2">
              ...
              <input
                ...
                className="flex-1 rounded-xl border border-slate-700 px-3 py-2 text-sm focus:border-indigo-400 disabled:bg-slate-800/60"
              />
              <button
                onClick={send}
